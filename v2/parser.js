...
(function (global) {
  ...
  // ---------------------------------------------------------------------------
  // Confidence + validation (consumed by popup.js for the LLM hybrid pipeline)
  // ---------------------------------------------------------------------------
  ...
  function confidenceScore(record) {
    ...
    for (const f of fields) {
      if (record[f] && String(record[f]).trim()) filled += 1;
    }
  ...
  function mergeRecords(base, override, overrideLabel) {
    const fields = [
      "vendor",
      "invoiceNumber",
      "invoiceDate",
    ...
    const provenance = Object.assign({}, base && base.fieldSources);
    for (const f of fields) {
      const ov = override && override[f];
      if (ov && String(ov).trim()) {
        out[f] = String(ov).trim();
        provenance[f] = overrideLabel || "override";
      } else if (!provenance[f] && base && base[f]) {
        provenance[f] =
          (base.fieldSources && base.fieldSources[f]) || "heuristic";
      }
    }
